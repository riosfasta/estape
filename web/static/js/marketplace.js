export function createMarketplace({ api, state, shell, app, esc, icons, uploadResizedImage, openEmbeddedCheckout, cropMarketplaceImage, imagePreviewURL }) {
  ...
  async function dashboard() {
    ...
    bindForm("#marketProfile", async form => {
      ...
      await api("/api/marketplace/profile", { method: "PUT", body: JSON.stringify({ name: values.name, title: values.title, bio: values.bio, country: values.country, location: values.location, skills: skillsValue(form), photo: url, availability: values.availability, portfolio_photos: portfolioPhotos, portfolio_details: media.portfolio.map(item => ({ photo: item.url, title: item.title || "", url: item.projectURL || "", description: item.description || "" })), youtube_urls: String(values.youtube_urls || "").split(/\r?\n/).map(url => url.trim()).filter(Boolean), public: !!values.public, consent: !!values.consent }) });
  ...
  async function wallet() {
    ...
    page("Wallet", `${heading("YOUR MONEY", "Wallet & payments", "All balances are in USD. Track hiring funds, earnings and settlement requests.")}${captureMessage ? `<p class="market-notice">${esc(captureMessage)}</p>` : ""}${params.has("cancelled") ? '<p class="market-notice">Checkout cancelled. No wallet credit was added.</p>' : ""}
      ...
      <section class="panel"><h2>Payment history</h2>${data.transfers.length ? `<div class="market-table-wrap"><table><thead><tr><th>Date</th><th>Type</th><th>Amount</th><th>Transaction fee</th><th>Status / reference</th></tr></thead><tbody>${data.transfers.map(t => `<tr><td>${esc(date(t.created_at))}</td><td>${esc(t.kind)}</td><td>${money(t.amount)}${t.status === "paid" ? `<small>Net sent ${money(t.amount - t.fee)}</small>` : ""}</td><td>${money(t.fee)}</td><td>${badge(t.status)}${t.dispute_status ? `<span class="badge" style="background:#fee2e2;color:#991b1b">Dispute: ${esc(t.dispute_status)}</span>` : ""}${t.seller_protection_status && t.seller_protection_status !== "ELIGIBLE" ? `<span class="badge" style="background:#fef3c7;color:#92400e" title="Seller Protection: ${esc(t.seller_protection_status)}">⚠️ Risk: ${esc(t.seller_protection_status)}</span>` : ""}${t.risk_warning ? `<small class="danger" style="display:block;color:#e11d48;font-weight:600">⚠️ ${esc(t.risk_warning)}</small>` : ""}${t.payment_reference ? `<small style="overflow-wrap:anywhere">Reference: ${esc(t.payment_reference)}</small>` : ""}<small>${esc(t.external_id || "")}</small>${t.kind === "topup" && t.status === "pending" ? `<button class="btn" data-capture="${t.id}">Verify PayPal payment</button>` : ""}</td></tr>`).join("")}</tbody></table></div>` : empty("No payments yet.")}</section><p><a href="/marketplace/privacy">Marketplace privacy & payment terms</a></p>`);
    bindForm("#marketTopup", async form => { const result = await post("/api/marketplace/topup", { amount: cents(new FormData(form).get("amount")) }); await openEmbeddedCheckout({ api, title: "Hiring balance", description: "Add funds to hire freelancers. Unused, unreserved balance is refundable; payment and refund transaction costs may be deducted.", amount: result.amount, orderID: result.order_id, captureURL: `/api/marketplace/topup/${encodeURIComponent(result.transfer_id)}/capture`, fallbackURL: result.url, onSuccess: async (captureRes) => { await wallet(); if (captureRes?.risk_warning) { message("Payment verified with warning: " + captureRes.risk_warning); } else { message("Payment verified and wallet credited."); } } }); });
    ...
    bindButtons("[data-capture]", async b => { const res = await post(`/api/marketplace/topup/${b.dataset.capture}/capture`); await wallet(); if (res?.risk_warning) { message("Payment verified with warning: " + res.risk_warning); } else { message("Payment verified."); } });
  }
