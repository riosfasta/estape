export function createMarketplace({ api, state, shell, app, esc, icons, uploadResizedImage }) {
  ...
  async function dashboard() {
    ...
    page("My profile", `${heading("YOUR MARKETPLACE PROFILE", "Build your next opportunity", "Hire talent, find work, and grow your reputation.", `<a class="btn" href="/marketplace/jobs">My jobs & offers</a><a class="btn primary" href="/find-jobs">Find work</a>`)}${notice()}
      ...
      <label class="field">Profile photo<input type="file" name="photo_file" accept="image/jpeg,image/png,image/webp"></label>${skillPicker(p.skills)}
    ...
    bindForm("#marketProfile", async form => {
      ...
      if (values.photo_file?.size) { const result = await uploadResizedImage(values.photo_file); url = result.url; }
      await api("/api/marketplace/profile", { method: "PUT", body: JSON.stringify({ name: values.name, title: values.title, bio: values.bio, country: values.country, location: values.location, skills: skillsValue(form), photo: url, public: !!values.public, consent: !!values.consent }) });
      await dashboard(); message("Profile saved.");
    });
    bindForm("#marketIdentity", async form => { const fd = new FormData(form); fd.set("consent", "true"); await api("/api/marketplace/identity", { method: "POST", body: fd }); await dashboard(); message("ID submitted for private review."); });
