...
func (s *Server) socialAuthSuccess(c *gin.Context, access string, refresh string, created bool) {
	...
	htmlBody := fmt.Sprintf(`<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Google Login</title></head><body><p>%s</p><script>localStorage.setItem("pinflow_access", %s);localStorage.setItem("pinflow_refresh", %s);window.location.replace("/dashboard");</script></body></html>`, html.EscapeString(message), accessJSON, refreshJSON)
