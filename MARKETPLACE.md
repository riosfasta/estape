...
## Task-board freelancer help and profile media

Website task boards provide Find FH with a grid/list picker, skills, country, availability, completed-job ratings and job-count filters. Owners review a task description before publishing a funded marketplace job and sending an invitation. Source task IDs are stored privately after task and website access checks. Existing jobs can receive further invitations; acceptance is followed by the existing manual hiring decision.

Profile and ID images require local crop/preview confirmation. Portfolio supports 12 photos and 6 validated YouTube video links. New images are limited to 500 KiB each, decoded and checked server-side (JPG/JPEG/PNG/WebP). Existing unchanged profile photos remain compatible. Public profile output includes portfolio and effective availability; ID bytes remain private. Availability choices are available, busy and running_project. Active jobs override the chosen status, and hire reservations also check availability atomically.
